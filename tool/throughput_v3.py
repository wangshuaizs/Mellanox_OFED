...
def plotfig(y_dict, axis_min, axis_max):
...
def main():
    ...
    parser.add_argument('-f', '--file', type=str, default="/home/shuai/test/0.log")
    parser.add_argument('-o', '--outputfile', type=str, default="/home/shuai/test/modified_0.log")
    ...
    print(" start time (ns)    end time (ns)         sqe index   length (Bytes)", file=outputf)
    ...
    while 1 :
        ...
        if int(line[0]) == 0 and int(line[1]) == 0:
            ...
            for i in xrange(distance) :
                ...
                print("%16s  %16s %16s %16d" % (modifing_line[0], modifing_line[1], modifing_line[2], 0 if byte == 2147483648 else byte), file=outputf)
    ...
    plotfig(throughput, axis_min, axis_max)
